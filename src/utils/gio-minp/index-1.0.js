...
const proxyVue = (t, e) => {
  void 0 !== t.mixin && t.mixin({
    ...
    beforeMount: function () {
      ...
      ("page" === t.mpType && "page" === this.mpType && t.$mp.page && (e.vueRootVMs[t.$mp.page.route || t.$mp.page.is] = t, 
        -1 !== ["wx"].indexOf(gioGlobal.gio__platform) && VdsInstrumentAgent.instrumentPageComponent(t.$mp.page)))
...
VdsInstrumentAgent = {
  ...
  initPlatformInfo(t) {
    VdsInstrumentAgent.appHandlers = t.appHandlers, 
    VdsInstrumentAgent.pageHandlers = t.pageHandlers, 
    VdsInstrumentAgent.actionEventTypes = t.actionEventTypes, 
    VdsInstrumentAgent.originalApp = t.originalApp, 
    VdsInstrumentAgent.originalPage = t.originalPage
  },
  initInstrument: function (t) {
    if (VdsInstrumentAgent.initPlatformInfo(gioGlobal.platformConfig), 
...
class Observer {
  ...
  pageListener(t, e, i) {
    ...
    if (t.route || (t.route = this.info.getPagePath(t)), this.growingio.vdsConfig.debug && console.log("Page.", t.route, "#", e, Date.now()), e === n.pageShow) {
      const e = getDataByPath(t, "$page.query");
      Utils.isEmpty(e) || "quickApp" !== gioGlobal.gio__platform || this.currentPage.addQuery(t, e), 
      this.isPauseSession ? this.isPauseSession = !1 : (this.currentPage.touch(t), this.useLastPageTime && this.currentPage.equalsPage(this.lastPageEvent) && (this.currentPage.time = this.lastPageEvent.tm, this.useLastPageTime = !1), this.sendPage(t))
  ...
  actionListener(t, e) {
    ...
    this.growingio.vdsConfig.debug && console.log("Click on ", e, Date.now()), 
    -1 !== i.click.indexOf(t.type) ? (this.sendClick(t, e), "getuserinfo" === t.type && getDataByPath(t, "detail.userInfo") && this.setVisitor(t.detail.userInfo)) : -1 !== i.change.indexOf(t.type) ? this.sendChange(t, e) : -1 !== i.submit.indexOf(t.type) && this.sendSubmit(t, e)
  ...
  sendVisitEvent(t, e) {
    this.info.getSystemInfo().then(i => {
      ...
      if (this.growingio.vdsConfig.appVer && (o.cv = this.growingio.vdsConfig.appVer + ""), t.length > 0) {
        let i = t[0];
        o.p = i.path || this.info.getPagePath(), 
        Utils.isEmpty(i.query) || (o.q = this.currentPage._getQuery(i.query)), 
        o.ch = `scn:${this.info.scnPrefix}${this.scene}`, 
        i.referrerInfo && i.referrerInfo.appId && getDataByPath(i, "referrerInfo.appId") && (o.rf = i.referrerInfo.appId), 
        this.info.getNetworkType().then(t => {
...
class GrowingIO {
  ...
  init(t, e, i) {
    ...
    this.start = !0, 
    i.vue && (this.vueRootVMs = {}, proxyVue(i.vue, this)),
    this._start()
    )
  }
  _start() {
    ...
    try {
      gioGlobal && platformConfig$1.canHook && (platformConfig$1.hooks.App && (gioGlobal.App = App), 
      platformConfig$1.hooks.Page && (gioGlobal.Page = Page))
    } catch (t) {}
  }
  login(t) {
  ...
  upload(t) {
    this.vdsConfig.debug && console.info("generate new event", JSON.stringify(t, 0, 2)), gioEmitter.emitEvent("upload", [t]), this.uploader.upload(t)
  }
  forceFlush() {
...
class BaseUserStorage {
  constructor(t) {
    ...
    this.userTagDuration = 432e7, 
    this.eventTagDuration = 864e5, 
